...
def calc_eval(exp):
    ...
    if isinstance(exp, Pair):
        ...
        if operator == 'and': # and expressions
            ...
            return calc_apply(OPERATORS[operator] if operator in OPERATORS else bindings[operator], operands)  # UPDATE THIS FOR Q2, what is type(operator)?
...
def floor_div(args):
    ...
    while args.rest is not nil:
        value = calc_eval(args.first) // calc_eval(args.rest.first)
        args = Pair(value, args.rest.rest)
    return value
...
def eval_and(expressions):
    ...
    return eval_and(expressions.rest) if (calc_eval(expressions.first) or calc_eval(expressions.first) is 0) else False
